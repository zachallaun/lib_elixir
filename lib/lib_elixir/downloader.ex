# From elixir_make, same license as Elixir:
# https://github.com/elixir-lang/elixir_make/blob/v0.8.4/lib/elixir_make/downloader/httpc.ex
defmodule LibElixir.Downloader do
  @moduledoc false
  ...
  def download(url) do
    ...
    case :httpc.request(:get, {url_charlist, []}, https_options, options) do
      ...
      other ->
        {:error, "error downloading #{url}: #{inspect(other)}"}
    end
  end
  ...
  defp warn_no_cacerts do
    ...
    Since lib_elixir could not detect a system
    ...
    3. Use OTP 25+ on an OS that has built-in certificate
       trust store.
    """)
  end
end
